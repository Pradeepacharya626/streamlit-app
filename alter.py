...
def capture_image1(key):
    ...
    if picture:
        ...
        files = {'file': (
        f'image{img_num}.jpg', cv2.imencode(".jpg", resized_img)[1].tobytes(),
        f'image{img_num}/jpeg')}
...
if __name__ == "__main__":
    ...
    if response:
        if response.status_code == 200:
            ...
            if posture == "PROPER":
                # add to text file
                increment_num = img_num + 1
                increment_num = str(increment_num)
                with open("capture.txt", "w") as file:
                    file.write(increment_num)
    ...
    if st.session_state.show_video == True :
        frame = st.empty()
        captured_image = None
